package touchui.core.action{
	...
	{	
		...
		public var noMouse:Boolean = false;
		...
		{	
			...
			this.removeEventListener(TouchEvent.MOUSE_DOWN, this.downEvent);						
			this.removeEventListener(TouchEvent.MOUSE_UP, this.upEvent);									
			...
			this.removeEventListener(MouseEvent.MOUSE_DOWN, this.mouseDownEvent);															
			this.removeEventListener(MouseEvent.MOUSE_UP, this.mouseUpEvent);	
			this.removeEventListener(MouseEvent.MOUSE_OVER, this.mouseRollOverHandler);
			this.removeEventListener(MouseEvent.MOUSE_OUT, this.mouseUpEvent);	
			this.removeEventListener(MouseEvent.DOUBLE_CLICK, this.doubleTap);										
			this.removeEventListener(Event.ENTER_FRAME, this.update);					
			trace('ROTATE/SCALE DESTROYED -----------------------------------------------!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!');
		}	
		...
		public function RotatableScalable()
		{
			...
			this.addEventListener(MouseEvent.MOUSE_OUT, this.mouseUpEvent, false, 0, true);	
			}
			
			this.addEventListener(MouseEvent.DOUBLE_CLICK, this.doubleTap, false, 0, true);										
		...
		public function addBlob(id:Number, origX:Number, origY:Number):void
		{
			...
			else if(blobs.length == 2)
			{
				...
				var tuioobj1:TUIOObject = TouchAPI.getObjectById(blob1.id);
			...
			for(var i:uint=0; i<blobs.length; i++)
			{
				if(blobs[i].id == id) 
				{
					...
					if(blobs.length == 1) 
					{
						...
						var tuioobj1:Object = TouchAPI.getObjectById(blob1.id);
					...
					if(blobs.length >= 2) {
						...
						tuioobj1 = TouchAPI.getObjectById(blob1.id);
		...
		{		
			...
			//DoubleTap Gesture
			var tuioobj:Object = TouchAPI.getObjectById(e.ID);
...
//			e.stopPropagation();	
		...
		public function mouseRollOverHandler(e:MouseEvent):void
		{
		//
		}
		
		public function mouseRollOutHandler(e:MouseEvent):void
		{
			if(!noMove)
			{
			//this.stopDrag();	
		...
		public function getAngleTrig(X:Number, Y:Number): Number
		{
			if (X == 0.0)
			{
				if(Y < 0.0)
		...
		public function released(dx:Number, dy:Number, dang:Number):void
		{
		}
		...
		private function update(e:Event):void
		{
			...
			if(state == "dragging")
			{
				var tuioobj:Object = TouchAPI.getObjectById(blob1.id);
...
//				trace(curPt.y + " " + blobs[0].origX);
			...
			} else if(state == "rotatescale")
			{
				...
				var tuioobj2:TUIOObject = TouchAPI.getObjectById(blob2.id);
