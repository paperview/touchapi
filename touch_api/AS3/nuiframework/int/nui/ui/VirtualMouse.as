package nui.ui {
	...
	public class VirtualMouse extends EventDispatcher {
		...
		public function set stage(s:MovieClip):void {
		 ...
		 * 		the virtual mouse.
		 */
		public function VirtualMouse(stage:MovieClip = null, startX:Number = 0, startY:Number = 0) {
			this.stage = stage;
		...
		private function handleUpdate(event:Event):void {
			...
			if (lastLocation.x != location.x || lastLocation.y != location.y) {
				...
				var withinStage:Boolean = Boolean(location.x >= 0 && location.y >= 0 && location.x <= stage.width && location.y <= stage.height);
