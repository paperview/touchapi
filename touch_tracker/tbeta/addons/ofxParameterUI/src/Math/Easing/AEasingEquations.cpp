...
float AEasingEquations::tickToFrac( int _tick, int _maxTick, int _fadeLength )
{
	...
	else if( (_tick >= 0) && (_tick <= _maxTick) )
	{
		if( _tick < _fadeLength ) 
		{ 
			tmpA = (float)_tick / (float)_fadeLength; 
		}
		else if ( _tick > (_maxTick-_fadeLength) ) 
		{ 
			tmpA =  1.0f - ((float)(_tick-(_maxTick-_fadeLength)) / (float)_fadeLength) ; 
		}
		else 
		{ 
			tmpA = 1.0f; 
